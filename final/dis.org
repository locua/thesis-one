...
where patterns can be combinations of shapes and shapes which make up patterns.
...
inspired this one were physically unavailable, being in the uk. Paperprograms
was available to download but as described below it was not suitable for this
idea. The objective was to aim for something lower level in both language and
theory. Create a ground system with which to build many different types of
...
implemented bl ob detection on the GPU I also found to be slow and unstable
...
*** Improvements
** Survey?
* Debugging and problem solving ** Main technical issues ** Problem solving
